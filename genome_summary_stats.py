...
def assess_genome_quality(file_path):
	...
	sequence_lengths.sort(reverse=True) #sorting occurs in place, which allows the shortest and longest sequences to be easily
	#retrieved, and aids the calculation of N50
	...
	try:
		#this ensures that genomes with ambigious nucleotides (aside from "N") are removed. Other genomes will still run as we 
		#are spawning and running multiple processes with concurrent.futures
		assert total_bp == (nucleotide_counts.get('C') +
		nucleotide_counts.get('G') +
		nucleotide_counts.get('T') + 
		nucleotide_counts.get('A') +
		nucleotide_counts.get('N')
		), f"Assertion Error: {genome_name}.fasta contains invalid characters and was ignored"
...
#calculating N50 
	...
	print(f"genome name:{genome_name}\t"
	      f"number of sequences:{number_of_sequences:,}\t"
	      f"GC:{GC:.2f}%\t N50:{N50:,}\t"
	      f"longest sequence:{sequence_lengths[0]:,}\t"
	      f"shortest sequence:{sequence_lengths[-1]:,}\t"
	      f"total bp:{total_bp:,}"
	     )

def main():
	fasta_files = [files for files in glob.glob(f"{args.file_path}/*.fasta") if "GD" in files] #GD is an internal keyword
